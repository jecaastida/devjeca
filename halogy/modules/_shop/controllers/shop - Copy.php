...
class Shop extends MX_Controller {
	...
	function index()
	{
	// if ($categs = $this->shop->get_categories()) {
            // $first_categ = $categs[0];
            // $first_categ_query_string = $first_categ['catSafe'];
            // $shop_redirect = '/shop/browse/' . $first_categ_query_string;
        // } else {
            // $shop_redirect = '/no-categories';
        // }
            // redirect($shop_redirect);		
	// 
    }
	function featured()
	{
	...
	function viewproduct($productID)
	{
		...
		$output['product:image-path'] = $this->config->item('staticPath').'/images/nopicture.jpg';
                }
                $mainImage = $_SERVER['DOCUMENT_ROOT'] . $output['product:image-path'];
                if (is_file($mainImage) && is_readable($mainImage)) {
                $image_size_info = getimagesize($mainImage);
                $image_width = $image_size_info[0];
                     if ($image_width > 350) {
                     $output['product:image-width'] = '350px';
                     } else {
                     $output['product:image-width'] =  $image_width . 'px';
                     }
                    
                } else {
                     $output['product:image-width'] = '300px';
                }
		...
		$data['variation3'] = $this->shop->get_variations($productID, 3);		
                */
                ...
                $data['variations'] = $this->shop->get_variations($productID, '');
		...
		$output['product:category'] = (isset($category) && $category) ? $category['catName'] : '';
                ...
                if($this->session->userdata('groupID') != "8"){
                    if ($product['sale_price'] != 0) {
                        $tmpStr = "<p class='retail-price crossed-out'>Retail Price: <span>" . currency_symbol() . number_format($product['price'], 2) . "</span></p>
                ...
                $output['product:price'] = $tmpStr;
                
                $output['product:oversize'] = "";
                if($product['oversize'] == 1){
                    $output['product:oversize'] = "This product is oversized";
                }
		...
		$output['productID'] = $productID;
                //preference
                $output['product:preference'] = $this->shop->getPreference($productID);
		// display with cms layer
	...
	function cart($quantity = '', $id = '')
	{
		...
		{
			redirect('/shop/checkout_billing_address');
		}
        ...
        // $shipping_band =$this->shop->get_bands();
        $shippingModifier = $this->input->post('shippingModifier');
        
        if ($shippingModifier == 2){
             if ($user['state'] == 'MM'){
                // $shippingBand['bandName'] = 'LBC';
                $shippingBand = 2.33333;
            }
            if ($user['state'] == 'LUZ'){
                
                $shippingBand = 3.33333;
            }
            if ($user['state'] == 'VIS'){
                
                $shippingBand = 3.333333;
            }
            ...
            if ($user['state'] == 'MM'){
                // $shippingBand['bandName'] = 'Xend';
                $shippingBand = 1;
            }
            if ($user['state'] == 'LUZ'){
                
                $shippingBand = 1.66666;
            }
            if ($user['state'] == 'VIS'){
		...
		if ($shippingBand || $shippingModifier)
		{
			...
			{
				// get modifier name
				$row = $this->shop->get_modifier_by_multiplier($shippingModifier);
				
				$this->session->set_userdata('shippingNotes', $row['modifierName']);
		...
		$output['cart:subtotal'] = currency_symbol().number_format(@$data['subtotal'], 2);
                ...
                $output['cart:shipping_label'] = "";
                
                if($isOversize){
                    $data['postage'] = $data['postage'] * 2;
                    $output['cart:shipping_label'] = " with oversized product";
                }
        ...
        $this->form_validation->set_rules('shippingBand', 'Select Your Courier and Region', 'required');
		// load content
                //print_r($data);
                //exit();
	...
	function cart_summary($quantity = '', $id = '')
	{
		...
		else
		{
			...
			if ($quantity == 'remove')
			{
				...
				if ($this->input->get('redirect')) {
					// get cart updates after removing items
					$this->shop->load_cart();
		...
		// load content
		$output['cart:items'] = @$this->parser->parse('partials/cart_summary', $data, TRUE);
		$output['cart:bands'] = @$this->parser->parse('partials/bands', $data, TRUE);
		$output['cart:modifiers'] = @$this->parser->parse('partials/modifiers', $data, TRUE);
		$output['form:discount-code'] = $this->session->userdata('discountCode');
		$output['form:donation'] = ($this->session->userdata('cart_donation')) ? number_format($this->session->userdata('cart_donation'), 2, '.', '') : '';	
		...
		$output['page:title'] = 'Shopping Cart'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		// display with cms layer	
		$this->pages->view('shop_cart_summary', $output, TRUE);
	}
	...
	function checkout()
	{
		// get partials
		$output = $this->partials;
		//echo require 'recurly/checkout/recurly.php';
		// if the gateway is sagepay or authorize then we need to post first and then redirect
		if (count($_POST))
		{
            if($this->input->post('payment_method') == 'v1' ){
                
                $this->_create_order($this->input->post('orderID'));
                header("Location: /shop/success");
            }
            
			// if($this->site->config['shopGateway'] == 'recurly'){
                                
				// if(isset($_POST) && $_POST['recurly_token'] != ''){   
						
					// require 'recurly/checkout/recurly.php';
					// Required for the API
					// Recurly_Client::$apiKey = $this->siteVars['shop_api_key'];
	
					// Optional for Recurly.js:
					// Recurly_js::$privateKey = $this->siteVars['shop_private_key'];
					...
					// if($transactionNumber == ''){
						// die("ERROR: Transaction Number is not set or Gateway Key is incorrect.");
					// }
				
					// if($this->session->userdata('cartOrderID') == ''){
						// die("ERROR: Order ID is not set.");
					// }
					// $this->shop->response_data['reference_id'] = $transactionNumber;
					// $this->_create_order($this->session->userdata('cartOrderID'));
					// $this->session->unset_userdata('cartOrderID');
					// header("Location: /shop/success");
					// exit();
				// }
			// }
		...
		{
			redirect('/shop/checkout_method/'.$this->core->encode($this->uri->uri_string()));
		}		
		...
		if($this->site->config['shopGateway'] == 'recurly'){
			$output['shop:gateway'] = site_url().'shop/payment';
		...
		if ($this->site->config['shopGateway'] == 'paypalpro') $output['shop:gateway'] = $this->uri->uri_string();
		// set page title
	...
	function create_account($redirect = '')
	{
		...
		if (!$redirect)
		{
			$redirect = $this->core->encode('/shop/cart');
		}
	...
	function login($redirect = '')
	{
		...
		if (!$this->session->userdata($sessionName))
		{
			// login
				...
				if ($password_entered && $email_entered) {
					
					//unset these sessions (if previously set)
					$this->session->unset_userdata('sameAddress');
					$this->session->unset_userdata('billing_address_step');
					$this->session->unset_userdata('shipping_address_step');
					$this->session->unset_userdata('shipping_step');
					$this->session->unset_userdata('payment_method_step');
					$this->session->unset_userdata('guest_userID');
					$this->session->unset_userdata('transaxTransID');
					$this->session->unset_userdata('guest_sess_id');
					$this->session->unset_userdata('guest_checkout_type');
				}
				$username = array('field' => 'email', 'label' => 'Email address', 'value' => $email_entered);
	...
	function checkout_method($redirect = '')
	{
         
		// check if user is 'logged in' | (redirect to 'billing address' step)
		if ($this->session->userdata('session_user'))
		{
			redirect('/shop/checkout_billing_address');
		}
		...
		$sessionName = 'session_user';
		$redirect = $redirect ? $redirect : $this->core->encode('/shop/checkout_billing_address');
		if (!$this->session->userdata($sessionName))
		{
			// login
			$password_entered = '';
		...
		// set page title
		$output['page:title'] = 'Checkout Method'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		$output['signupRedirect'] = $redirect;	
		
		// cart info box (left panel)
		$output['cart_info_leftbar'] = $this->cart_info_leftbar();
		// display with cms layer
		$this->pages->view('shop_checkout_method', $output, TRUE);
	}
	
	function checkout_billing_address()
	{
		// get partials
		$output = $this->partials;
		
		// check if user is 'logged in' | check if user is a 'guest with a session id' (if not send them away from this controller)
		if (!$this->session->userdata('session_user') && !$this->session->userdata('guest_sess_id'))
		{
			redirect('/shop/checkout_method/'.$this->core->encode($this->uri->uri_string()));
		}
		// set object ID and validation codes(registered users)
		if ($this->session->userdata('session_user')) {
			$objectID = array('userID' => $this->session->userdata('userID'));	
			...
			$this->core->required = array(
				'firstName' => array('label' => 'First name', 'rules' => 'required|trim|ucfirst'),
				'lastName' => array('label' => 'Last name', 'rules' => 'required|trim|ucfirst'),
				'billingAddress1' => array('label' => 'Address1', 'rules' => 'required|required|trim|ucfirst'),
				'billingAddress2' => array('label' => 'Address2', 'rules' => 'trim|ucfirst'),
				'billingAddress3' => array('label' => 'Address3', 'rules' => 'trim|ucfirst'),
				'billingCity' => array('label' => 'City / State', 'rules' => 'required|trim|ucfirst'),
				'billingPostcode' => array('label' => 'ZIP/Postcode', 'rules' => 'required|trim|strtoupper'),
				'phone' => array('label' => 'Phone', 'rules' => 'required|trim')			
			);	
		}
		
		// set object ID and validation codes(guest users)
		if ($this->session->userdata('guest_sess_id')) {
			$objectID = array('sessionID' => $this->session->userdata('guest_sess_id'));	
			
			// for those guests who will register
			if ($this->session->userdata('guest_checkout_type') == 1) {
			$this->core->required = array(
				'firstName' => array('label' => 'First name', 'rules' => 'required|trim|ucfirst'),
				'lastName' => array('label' => 'Last name', 'rules' => 'required|trim|ucfirst'),
				'email' => array('label' => 'Email address', 'rules' => 'required|valid_email|unique[users.email]|trim'),
			...
			// for those guests who will not register
			if ($this->session->userdata('guest_checkout_type') == 2) {
			$this->core->required = array(
				'firstName' => array('label' => 'First name', 'rules' => 'required|trim|ucfirst'),
				'lastName' => array('label' => 'Last name', 'rules' => 'required|trim|ucfirst'),
				'email' => array('label' => 'Email address', 'rules' => 'required|valid_email|unique[users.email]|trim'),
		...
		// get values (registered users)
		if ($this->session->userdata('session_user')) {
			$data = $this->core->get_values('users', $objectID);
		}
		
		// get values (guest users)
		if ($this->session->userdata('guest_sess_id')) {
			$data = $this->core->get_values('guests', $objectID);
		}
		if ($this->input->post('password')) $this->session->set_userdata('guest_pw', $this->input->post('password'));
		
		// force postcode to upper case
		$this->core->set['billingPostcode'] = strtoupper($this->input->post('billingPostcode'));
		// security check
		if ($this->input->post('username')) $this->core->set['username'] = $data['username'];
		if ($this->input->post('premium')) $this->core->set['premium'] = $data['premium'];
		if ($this->input->post('siteID')) $this->core->set['siteID'] = $this->siteID;
		...
		// set shipping address if 'sameAddress' is checked (guest users)
		if (isset($_POST["sameAddress"])){
			if ($this->input->post('sameAddress') == "yes" && $this->session->userdata('guest_sess_id')) {
				$this->core->set['address1'] = $this->input->post('billingAddress1');
				$this->core->set['address2'] = $this->input->post('billingAddress2');
				$this->core->set['address3'] = $this->input->post('billingAddress3');
				$this->core->set['city'] = $this->input->post('billingCity');
				$this->core->set['state'] = $this->input->post('billingState');
				$this->core->set['postcode'] = $this->input->post('billingPostcode');
				$this->core->set['country'] = $this->input->post('billingCountry');
				
				// set sameAddress checkbox default value to 'checked'
				$this->session->set_userdata('sameAddress', 'checked');
			}
			elseif ($this->input->post('sameAddress') == "no") {
				$this->session->unset_userdata('sameAddress');
			}
		}
		$updated = 0;
		...
		// update (guest users)
		if ($this->session->userdata('guest_sess_id')) {
			if (count($_POST) && $this->core->update('guests', $objectID))
			{
				// get updated row session
				$guest = $this->shop->get_guest();
		...
		if ($updated == 1) {
			...
			// mark 'billing address' step as 'done'
			$this->session->set_userdata('billing_address_step', 'done');
			
			// mark 'shipping address' step as 'done' if 'sameAddress' is checked
			if ($this->input->post('sameAddress') == "yes" && $this->session->userdata('guest_sess_id')) {
				$this->session->set_userdata('shipping_address_step', 'done');
			}
			
			// redirect to 'payment method' step (if 'shipping' step is 'done')
			if ($this->session->userdata('shipping_step') == "done") {
				redirect('/shop/checkout_payment_method');	
			}
			
			// redirect to 'shipping' step (if 'shipping address' step is 'done')
			if ($this->session->userdata('shipping_address_step') == "done") {
				redirect('/shop/checkout_shipping');	
			}
			
			// redirect to 'shipping address' step (if 'billing address' step is 'done')
			if ($this->session->userdata('billing_address_step') == "done") {
				redirect('/shop/checkout_shipping_address');	
			}
		}
		// if user is logged-in, hide 'checkout method' step 
		$methodstyle = '';
		if ($this->session->userdata('session_user')) {
			$methodstyle = 'style="display:none"';
		}
		// populate template
		$output['form:firstName'] = set_value('firstName', @$data['firstName']);
		$output['form:lastName'] = set_value('lastName', @$data['lastName']);
		...
		$output['select:billingCountry'] = @display_countries('billingCountry', set_value('billingCountry', @$data['billingCountry']), 'id="billingCountry" class="formelement"');			
        $output['form:phone'] = set_value('phone', @$data['phone']);
		$output['methodstyle'] = $methodstyle;
		$output['form:email'] = '';
		$output['form:sameAddress'] = '';
		
		//additional fields (for guest users)
		if ($this->session->userdata('guest_sess_id')) {
			
			//email field
			$output['form:email'] = @form_label('Email', 'email');
			$output['form:email'] .= @form_input('email',set_value('email', $data['email']), 'id="email" class="formelement"');
			$output['form:email'] .= '<br class="clear" />';
			
			//sameAddress field
			$defaultval = FALSE;
			if ($this->session->userdata('sameAddress') == "checked"){ $defaultval = TRUE;}
			$output['form:sameAddress'] = @form_hidden('sameAddress', 'no');
			$output['form:sameAddress'] .= @form_checkbox('sameAddress', 'yes', $defaultval);
			$output['form:sameAddress'] .= 'My billing address is the same as my shipping address.<br class="clear" /><br/>';
			
			//password field (for those guests who will register)
			if ($this->session->userdata('guest_checkout_type') == 1) {
			$output['form:password'] = @form_label('Password', 'password');
			$output['form:password'] .= @form_password('password', '', 'id="password" class="formelement"');
			$output['form:password'] .= '<br class="clear" />';
			
			$output['form:confirmPassword'] = @form_label('Confirm Password', 'confirmPassword');
			$output['form:confirmPassword'] .= @form_password('confirmPassword', '', 'id="confirmPassword" class="formelement"');
			$output['form:confirmPassword'] .= '<br class="clear" />';
			}
		}
		// checkout progress (billing address)
		$output['form:billingprogress'] = "Pending";
		$output['form:billingAddressEdit'] = "";
		
		if ($this->session->userdata('billing_address_step') == "done") {
			$billingcontent = "<div>".$data['firstName']."</div>";
			$billingcontent .= "<div>".$data['lastName']."</div>";
			$billingcontent .= "<div>".$data['billingAddress1']."</div>";
			$billingcontent .= "<div>".$data['billingAddress2']."</div>";
			$billingcontent .= "<div>".$data['billingAddress3']."</div>";
			$billingcontent .= "<div>".$data['billingCity'].", ".$data['billingState'].", ".$data['billingPostcode']."</div>";
			$billingcontent .= "<div>".$data['billingCountry']."</div>";
			$billingcontent .= "<div>T: ".$data['phone']."</div>";
			$output['form:billingprogress'] = $billingcontent;
			$output['form:billingAddressEdit'] = "<a href='".site_url('shop/checkout_billing_address#')."'>edit</a>";
		}
		
		// load errors
		$output['errors'] = (validation_errors()) ? validation_errors() : FALSE;		
		// set page title
		$output['page:title'] = 'Billing Address'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		
		// cart info box (left panel)
		$output['cart_info_leftbar'] = $this->cart_info_leftbar();
		// display with cms layer	
		$this->pages->view('shop_checkout_billing_address', $output, TRUE);
	}
	function checkout_shipping_address()
	{
		...
		// set object ID (registered users)
		if ($this->session->userdata('session_user')) {
			$objectID = array('userID' => $this->session->userdata('userID'));	
		}
		
		// set object ID (guest users)
		if ($this->session->userdata('guest_sess_id')) {
			$objectID = array('sessionID' => $this->session->userdata('guest_sess_id'));	
		}
		// required
		...
		if ($updated == 1) {
			...
			// mark 'shipping address' step as 'done'
			$this->session->set_userdata('shipping_address_step', 'done');
		...
		// checkout progress (shipping address)
		$output['form:shippingprogress'] = "Pending";
		$output['form:shippingAddressEdit'] = "";
		if ($this->session->userdata('shipping_address_step') == "done") {
			$shippingcontent = "<div>".$data['address1']."</div>";
			$shippingcontent .= "<div>".$data['address2']."</div>";
			$shippingcontent .= "<div>".$data['address3']."</div>";
			$shippingcontent .= "<div>".$data['city'].", ".$data['state'].", ".$data['postcode']."</div>";
			$shippingcontent .= "<div>".$data['country']."</div>";
			$shippingcontent .= "<div>T: ".$data['phone']."</div>";
			$output['form:shippingprogress'] = $shippingcontent;
			$output['form:shippingAddressEdit'] = "<a href='".site_url('shop/checkout_shipping_address#')."'>edit</a>";
		}
		// load errors
		$output['errors'] = (validation_errors()) ? validation_errors() : FALSE;		
		// set page title
		$output['page:title'] = 'Shipping Address'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		
		// cart info box (left panel)
		$output['cart_info_leftbar'] = $this->cart_info_leftbar();
		// display with cms layer	
		$this->pages->view('shop_checkout_shipping_address', $output, TRUE);
	}
	...
	function checkout_shipping()
	{
		...
		if ($this->input->get('continue') == "true") {
			
			// mark 'shipping' step as 'done'
			$this->session->set_userdata('shipping_step', 'done');
		...
		$output['form:shipping'] = "Pending";
		$output['form:shippingEdit'] = "";
		
		if ($this->session->userdata('shipping_step') == "done") {
			$output['form:shipping'] = "Confirmed";
			$output['form:shippingEdit'] = "<a href='".site_url('shop/checkout_shipping#')."'>edit</a>";
		}
		...
		$output['cart:discounts'] = ($data['discounts'] > 0) ? currency_symbol().number_format(@$data['discounts'], 2) : '';
		if ($data['discounts'] == 0) {
		$output['cart:discounts'] =  currency_symbol().number_format(0, 2);
		}
		...
		$output['cart:modifiers'] = @$this->parser->parse('partials/modifiers', $data, TRUE);
		// set page title
		$output['page:title'] = 'Shipping'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		...
		$output['methodstyle'] = $methodstyle;
		// display with cms layer	
		$this->pages->view('shop_checkout_shipping', $output, TRUE);
	}
	...
	function checkout_payment_method()
	{
		...
		$output['form:paymentmethod'] = "Pending";
        $output['form:paymentEdit'] = "";		
		if ($this->session->userdata('payment_method_step') == "done") {
			
			$output['form:paymentEdit'] = "<a href='".site_url('shop/checkout_payment_method#')."'>edit</a>";		
			
			if ($this->session->userdata('paymentmethod') == 1){
			$output['form:paymentmethod'] = "Paypal";
			}
			
			if ($this->session->userdata('paymentmethod') == 2){
			$output['form:paymentmethod'] = "BDO";
			}
            
            if ($this->session->userdata('paymentmethod') == 3){
			$output['form:paymentmethod'] = "OTC";
			}
            
            if ($this->session->userdata('paymentmethod') == 4){
			$output['form:paymentmethod'] = "SMART MONEY";
			}
		}
		
		// get user data (registered users)
		if ($this->session->userdata('session_user')) {
			$user = $this->shop->get_user();
		}
		
		// get user data (guest users)
		if ($this->session->userdata('guest_sess_id')) {
			$user = $this->shop->get_guest();
		}
		...
		if (empty($_GET['token-id'])) {
			...
			if ($data = $this->shop->load_cart())
			{
				...
				// get transaction data (registered users)
				if ($this->session->userdata('session_user')) {
					$transaction = $this->shop->insert_transaction();
				}
				
				// get transaction data (guest users)
				if ($this->session->userdata('guest_sess_id')) {
					$transaction = $this->shop->insert_guest_transaction();
				}
		...
		// process transax payment (if payment form is submitted)
		// if ($this->input->post('transaxsubmission')) {
			
			//get total amount
			// if ($amtdata = $this->shop->load_cart())
			// {
				// $total_amt = number_format(($amtdata['subtotal'] + $amtdata['postage'] + $amtdata['tax']), 2);				
			// }
			...
			// $params['GatewayPassword'] = "Nelix123!";
			...
			// $params['Amount'] = $total_amt;
			// $params['OrderID'] = mt_rand(1,9999);
			// $params['FirstName'] = @$user['firstName'];
			// $params['LastName'] = @$user['lastName'];	
			// $params['Address1'] = @$user['billingAddress1']; 
			// $params['Address2'] = @$user['billingAddress2']; 
			// $params['City'] = @$user['billingCity']; 
			// $params['State'] = @lookup_state($user['billingState']); 
			// $params['Zip'] =  @$user['billingPostcode']; 
			// $params['Country'] = $user['billingCountry']; 
			// $params['Phone'] = @$user['phone']; 
			// $params['EMail'] = @$user['email']; 
			// $params['ShippingAddress1'] = @$user['address1']; 
			// $params['ShippingAddress2'] = @$user['address2']; 
			// $params['ShippingCity'] = @$user['city']; 
			// $params['ShippingState'] = @lookup_state($user['state']); 
			// $params['ShippingZip'] = @$user['postcode']; 
			// $params['ShippingCountry'] = @$user['country']; 
			
			// $proxy = $soapclient->getProxy();
			
			// $result = $proxy->ProcessTransaction(Array('objparameters' => $params));
			
			// $status_code = $result['ProcessTransactionResult']['STATUS_CODE'];
			// $status_msg = $result['ProcessTransactionResult']['STATUS_MSG'];
			// $trans_id = $result['ProcessTransactionResult']['TRANS_ID'];
			
			//generate random string
			// $randval = random_string('alnum');
			
			// mark 'payment method' step as 'done'
			// $this->session->set_userdata('payment_method_step', 'done');
			
			// chosen payment method (card)
			// $this->session->set_userdata('paymentmethod', 2);
			
			
			// approved
			// if ($status_code == 1 ) {
				
				//transax transaction id
				// $transax_transationID = $trans_id;
				// $this->session->set_userdata('transaxTransID', $transax_transationID);
				
				//'transaction approved' value
				// $paymentresult = 1;
				
				// $description = 'Transaction Approved';
				// $this->shop->add_transaxrecord($randval, $transax_transationID, $paymentresult, $description);
				// $this->_create_order($this->session->userdata('cartOrderID'));
				
				// redirect('/shop/_order_review?rnum='.$randval);
			...
			// } elseif($status_code == 2)  {
				...
				// $payment_error_msg = "<div class='error'><p>".$description."</p></div>";
		...
		// $transaxform = '
				// <form action="" method="POST">
					// <table>
						// <tr><td>Credit Card Number</td><td><INPUT type ="text" name="CCNumber" size="20" value=""> </td></tr>
						// <tr><td>Expiration Date</td><td><INPUT type ="text" name="CCExpDate" size="20" value=""> </td></tr>
						// <tr><td>CVV</td><td><INPUT type ="text" name="CVV" > </td></tr>
						// <tr><Td> <Input type="hidden" Name="transaxsubmission" value="1"><INPUT type ="submit" value="Continue" class="button"></td><td></td></tr>
					// </table>
				// </form>';
        ...
        $div3 = "'div3'";
        $div4 = "'div4'";
		$selectionform = ''.$payment_error_msg.'<h2>Select Payment Method</h2><form name="pForm">
		<input type="radio" name="payment_method" value="v1" onClick="showMeRBtn('.$div1.', this.name)" />Bank Deposit (BDO) <br/>
		<input type="radio" name="payment_method" value="value2" onClick="showMeRBtn('.$div2.', this.name)" />Paypal <br/>
        <input type="radio" name="payment_method" value="value3" onClick="showMeRBtn('.$div3.', this.name)" />Money Transfer (Cebuana Lhuilhier,Palawan Express, ML, LBC) <br/>
        <input type="radio" name="payment_method" value="value4" onClick="showMeRBtn('.$div4.', this.name)" />Smart Money <br/><br/>
		</form><br/><br/>';
		...
		// load content
		$output['cart:items'] = @$this->parser->parse('partials/cart', $data, TRUE);
		$output['paymentmethods'] = $selectionform;
		// $output['transaxcontent'] = $transaxform;		
		$output['shop:checkout'] = @$this->parser->parse('partials/checkout_forms', $data, TRUE);
		$output['methodstyle'] = $methodstyle;
		
		// prevent users from using the payment forms (if errors are present)
		if (validation_errors()) {
			$output['paymentmethods'] = "";
			$output['transaxcontent'] = "";		
			$output['shop:checkout'] = "";
		}
		
		// post to the same page if paypal pro
		if ($this->site->config['shopGateway'] == 'paypalpro') $output['shop:gateway'] = $this->uri->uri_string();
		// set page title
		$output['page:title'] = 'Payment Method'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		
		// cart info box (left panel)
		$output['cart_info_leftbar'] = $this->cart_info_leftbar();
		
		// display with cms layer	
		@$this->pages->view('shop_checkout_payment_method', $output, TRUE);		
	}
	...
	function checkout_order_review()
	{
		...
		// check if user is 'logged in' | check if user is a 'guest with a session id' (if not send them away from this controller)
		if (!$this->session->userdata('session_user') && !$this->session->userdata('guest_sess_id') && !$this->session->userdata('viewpage'))
		{
			redirect('/shop/checkout_method/'.$this->core->encode($this->uri->uri_string()));
		}
		// set object ID (registered users)
		...
		// transax payment info
		// if ($this->input->get('rnum') && $this->session->userdata('paymentmethod') == 2 ) {
			// if ($rec = $this->shop->get_transaxrecords($this->input->get('rnum')))
			// {
				// $transactionCode = $rec['transactionCode'];
				// $transactionResult = $rec['paymentresult'];
				// $transaxID = $rec['transaxtransactionID'];
				// $transactiondesc = $rec['transactiondesc'];
				
				// if ($transactionResult == 1) {
					// $transMsg = "<h2>Transaction was Approved.</h2><h2> Transax Transaction ID: ".$transaxID."</h2>";
				// }
				
				// allow users to view 'order review' page even if the session IDs are empty
				// $this->session->set_userdata('viewpage', 'active');
				
			// }
			
			// if ($transrec = $this->shop->get_transaction_id($transactionCode))
			// {
		...
		// paypal payment info
		if ($this->input->get('tx') && $this->input->get('st') == "Completed" && $this->input->get('item_number')) {
			$paypal_trans_id = $this->input->get('tx');
			$transactionCode = $this->input->get('item_number');
			
			if ($transrec = $this->shop->get_transaction_id($transactionCode))
			{
				$transactionID = $transrec['transactionID'];
				$transMsg = "<h2>Transaction was Approved.</h2><h2> Paypal Transaction ID: ".$paypal_trans_id."</h2>";
				
				// create order & update admin records.. 
				// this line can be removed if the site is using ipn to process the orders
				$this->_create_order($transactionCode);
			}
			
			// chosen payment method (paypal)
			$this->session->set_userdata('paymentmethod', 1);
			$this->session->set_userdata('payment_method_step', 'done');
		...
		if ($this->session->userdata('payment_method_step') == "done") {
			...
			if ($this->session->userdata('paymentmethod') == 2){
			$output['form:paymentmethod'] = "Debit / Credit Card";
			}
		...
		// user id (registered users)
		if ($this->session->userdata('session_user')) {
			$userIDval = $this->session->userdata('userID');
		}
		
		// user id (guest users)
		if ($this->session->userdata('guest_sess_id')) {
			$userIDval = $this->session->userdata('guest_userID');
		}
		// grab data and display
		if (!$order = $this->shop->get_order($transactionID, $userIDval))
		{
			show_error('Not a valid order!');
		}
		// grab data and display
			...
			foreach($item_orders as $item)
			{
				...
				if(is_array($item['variations'])){
					foreach($item['variations'] as $vart){
						$variations .= ' ('.$vart['name'].': '.$vart['variation'].')';
					}
				}
		...
		if ($this->session->userdata('guest_checkout_type') == 1) {
			...
			$this->shop->activate_guest_account($this->session->userdata('guest_userID'));
			
			// send email to guests (registration confirmation)
			// set header and footer
			$emailHeader = str_replace('{name}', trim($data['firstName'].' '.$data['lastName']), $this->site->config['emailHeader']);
			$emailHeader = str_replace('{first-name}', $data['firstName'], $emailHeader);
			$emailHeader = str_replace('{last-name}', $data['lastName'], $emailHeader);
			$emailHeader = str_replace('{email}', $data['email'], $emailHeader);
			$emailFooter = str_replace('{name}', trim($data['firstName'].' '.$data['lastName']), $this->site->config['emailFooter']);
			$emailFooter = str_replace('{first-name}', $data['firstName'], $emailFooter);
			$emailFooter = str_replace('{last-name}', $data['lastName'], $emailFooter);
			$emailFooter = str_replace('{email}', $data['email'], $emailFooter);
			$emailAccount = str_replace('{name}', trim($data['firstName'].' '.$data['lastName']), $this->site->config['emailAccount']);
			$emailAccount = str_replace('{first-name}', $data['firstName'], $emailAccount);
			$emailAccount = str_replace('{last-name}', $data['lastName'], $emailAccount);
			$emailAccount = str_replace('{email}', $data['email'], $emailAccount);			
		
			// send email
			$password = $this->session->userdata('guest_pw');
			$this->load->library('email');
			$this->email->from($this->site->config['siteEmail'], $this->site->config['siteName']);
			...
			$registrationMsg = "<h2>Your account has been set up. We have sent you an email. Thank you for registering with us.</h2><hr/>";
		}
		// if user is logged-in, hide 'checkout method' step 
		$methodstyle = '';
		if ($this->session->userdata('session_user')) {
		...
		$output['order:status'] = $order['trackingStatus'];		
		$output['order:notes'] = ($order['notes']) ? nl2br($order['notes']) : FALSE;
		$output['order:transMsg'] = $registrationMsg.$transMsg;
		$output['methodstyle'] = $methodstyle;
		...
		$this->session->unset_userdata('guest_sess_id');
		$this->session->unset_userdata('guest_checkout_type');
		$this->session->unset_userdata('guest_pw');
		// set page title
		$output['page:title'] = 'Order Review'.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');
		// display with cms layer	
		$this->pages->view('shop_checkout_order_review', $output, TRUE);
	}
	...
	function guestcheckout(){
		...
		$this->shop->guest_session($this->input->post('guestcheckout'));
		redirect('/shop/checkout_billing_address');
	}
	...
	function cart_info_leftbar()
	{
		...
		// if($isOversize){
			// $data['postage'] = $data['postage'] * 2;
		// }
		$discountvalue = ($data['discounts'] > 0) ? currency_symbol().number_format(@$data['discounts'], 2) : '';
		if ($data['discounts'] == 0) {
			$discountvalue =  currency_symbol().number_format(0, 2);
		}
		$subtotal = currency_symbol().number_format(@$data['subtotal'], 2);
		$shippingrate = currency_symbol().number_format(@$data['postage'], 2);
		$statetax = ($data['tax'] > 0) ? currency_symbol().number_format(@$data['tax'], 2) : '';
		$carttotal = currency_symbol().number_format((@$data['subtotal'] + @$data['postage'] + @$data['tax']), 2);
		
		$output .= "<div class='leftbar_orderinfo_box'>";
		$output .= "<div class='leftbar_orderinfo'><span style='width:100px;float:left'>Discount:</span><span style='float:right'>".$discountvalue."</span></div>";
		$output .= "<div class='leftbar_orderinfo'><span style='width:100px;float:left'>Sub total:</span><span style='float:right'>".$subtotal."</span></div>";
		$output .= "<div class='leftbar_orderinfo'><span style='width:100px;float:left'>Shipping:</span><span style='float:right'>".$shippingrate."</span></div>";
		
		if ($data['tax'] > 0){
		$output .= "<div class='leftbar_orderinfo'><span style='width:100px;float:left'>Tax:</span><span style='float:right'>".$statetax."</span></div>";
		}
		$output .= "<div class='leftbar_orderinfo'><span style='width:100px;float:left'>TOTAL:</span><span style='float:right'>".$carttotal."</span></div>";
		$output .= "</div>";
		return $output;
	}
	function reset($key = '')
	...
	function review($productID)
	{
		...
		$this->core->required = array(
			...
			'email' => array('label' => 'Email', 'rules' => 'required|valid_email'),
			'review' => 'Review',
			'captcha' => array('label' => 'Captcha', 'rules' => 'required|callback_captcha_check')				
		);
		// get values
		$output = $this->core->get_values();	
		...
		$output['form:review'] = $this->input->post('review');	
		// set captcha
		$captcha = $this->shop->captcha();
		$output['captcha'] = $captcha['image'];
		// set title
	...
	function _create_order($orderID = '')
	{
		...
		{	
			...
			if ($order['variations']) {
				foreach($order['variations'] as $vart){
					$variationHTML .= ' ('.$vart['name'].': '.$vart['variation'].')';
				}
				
				if ($order['oversize'] == 1){ 
					$variationHTML .= ' (Oversized Product)';                           
				}
			}
			/*
			// get variation 1
			...
			if ($order['fileID'])
			{
				...
				$downloadBody .= $order['productName']."\n".site_url('/files/'.$this->core->encode($file['fileRef'].'|'.$transactionID))."\n\n";
                        ...
                        if($this->session->userdata('groupID') != "8"){
                            ...
                            if ($order['wholesale_price']!= 0) {
                               $order['price'] = $order['wholesale_price'];
                            } 
                        }*/
