...
class Shop_model extends CI_Model {
	...
	function __construct()
	{
        ...
        function getPreference($productID, $arr= array("1" => '1', "2" => '1')){
                $query = $this->db->query("SELECT bm.*, b.brandTitle, b.order_id
                                    FROM ".$this->db->dbprefix."brands_map bm
                                    LEFT JOIN ".$this->db->dbprefix."brands   b     ON b.brandID= bm.brandID
                                    WHERE bm.productID =$productID
                                    ORDER BY b.order_id DESC, b.brandTitle");
                $str = "";
                if (@$query->num_rows() > 0){
                    ...
                    for($x=1; $x<3; $x++){
                        ...
                        $str .= "<p>Preference $label: <select name='preference$x' id='preference$x' class='preference'>";
                        foreach($query->result() as $row){

                            $selected = "";

                            if($arr[$x] == $row->brandID){
                                $selected = " selected='selected' ";
                            }

                            $str .= "<option $selected value='".$row->brandID."'>".$row->brandTitle."</option>";
                        }
	...
	function get_categories($catID = '')
	{
        ...
        function getGroupName($group_id){
                ...
                $query = $this->db->get('permission_groups', 1);
  ...
  function postage_progress()
   {
    
    $this->db->select('total, cost');
    $this->db->from('ha_shop_postages');
	$this->db->where('siteID',  $this->siteID);
	$this->db->order_by('total', 'asc');
    $query = $this->db->get();
    
    $postages = $query->result();
    
     if(is_array($postages))
     {
	  $postage_notes = '';
      foreach($postages as $ship_rates)
      {
		$postage_notes .= "<div>*When cart total reaches <strong>".currency_symbol().number_format(($ship_rates->total), 2)."</strong>, then <strong>".currency_symbol().number_format(($ship_rates->cost), 2)."</strong> shipping rate will be applied.</div>";
      }
      
      return $postage_notes;
     }
   }	   
	...
	function get_variations($productID, $type = '')
	{
        ...
        function get_all_variations($variationIDS){
                $arrIds = explode(',',$variationIDS);
                $this->db->where_in('variationID', $arrIds);
                $query = $this->db->get('shop_variations');
		//echo $variationID;
		if ($query->num_rows())
		{
	...
	function unpack_item($key, $quantity)
	{
		...
		$product = $this->get_product($keys['productID']);	
                //echo $keys['variation1'].'<br />';
                $variations = @$this->get_all_variations($keys['variation1']);
		...
		$variation3 = @$this->get_variation($keys['variation3']);
                $preference = @$this->getPreference($keys['productID'], array("1" => $keys['preference1'],"2" => $keys['preference2']));
		...
		$product['imageName'] = (file_exists('.'.$thumbPath)) ? $thumbPath : $imagePath; 
                */

                $thumb = $this->uploads->load_image($keys['productID'], true, true);
                $product['imageName'] = $thumb['src'];
		...
		$item = @array( 'productID' => $keys['productID'],
                                'catID' => $product['catID'],
                                ...
                                'ourcost_price' => $product['ourcost_price'],
                                'quantity' => $quantity,
                                'variation1' => $variation1['variation'],
                                'variation1Price' => $variation1['price'],
                                'variation2' => $variation2['variation'],
                                'variation2Price' => $variation2['price'],
                                'variation3' => $variation3['variation'],
                                'variation3Price' => $variation3['price'],
                                ...
                                'stock' => $product['stock'],
                                'preference1' => $keys['preference1'],
                                'preference2' => $keys['preference2'],
                                'preferenceHTML' => $preference,
                                'productThumb' => $product['imageName'], );
	...
	function load_cart()
	{
		...
		if ($cartSession)
		{
			...
			foreach ($cartSession as $key => $quantity)
			{
				...
				{
					// discount is present
					$this->session->set_userdata('summarydiscount', 1);
				...
				if (empty($discount))
				{
					...
					$this->session->set_userdata('summarydiscount', 0);
				}
			...
			// product qty
			$this->session->set_userdata('product_qty', $product_qty);

			// calculate discount on total
			if (isset($discount) && $discount['type'] == 'T')
			{
				// set total discounted amount
			...
			if ($this->siteVars['shopTax'] > 0)
			{
				...
				// tax is present
				$this->session->set_userdata('summarytax', 1);
			}
			
			if (empty($tax))
			{
				// tax is not present
				$this->session->set_userdata('summarytax', 0);
			}
	...
	{   
		...
		if ($productID)
		{
			$cart = $this->session->userdata('cart');
                        ...
                        $variationIDs = "";
                        $variationIDs = @implode(',',$this->input->post('variation'));
                        
                        //print_r($variationIDs);
              	
			$variation1 = ($variationIDs) ? $variationIDs : $variationIDs;
	...
	function add_transaxrecord($randval, $transax_transationID, $paymentresult, $description)
	{
		$record = array(
		   'transactionCode' => $this->session->userdata('cartOrderID') ,
		   'transaxtransactionID' => $transax_transationID ,
		   'randomvalue' => $randval ,
		   'paymentresult' => $paymentresult ,
		   'transactiondesc' => $description ,
		);
		
		$this->db->insert('ha_transax_records', $record); 
	}

	function guest_session($guestcheckouttype)
	{	
		// require guests to select a 'guest checkout' method
		if (!$guestcheckouttype){
			redirect('/shop/checkout_method');
			exit();
		}
		
		if ($guestcheckouttype == 1) {
		// guest will register and checkout together
		$this->session->set_userdata('guest_checkout_type', 1);
		}
		
		if ($guestcheckouttype == 2) {
		// guest will checkout without registering 
		$this->session->set_userdata('guest_checkout_type', 2);
		}
		
		// prevent guests from generating a new session if they already have one
		...
		if (!$this->session->userdata('guest_sess_id')) {
			$guest_sess_id = random_string('alnum');
			$this->session->set_userdata('guest_sess_id', $guest_sess_id);
			...
			$this->db->insert('ha_guests', $guestrecord); 
		}
	}

	function activate_guest_account($guest_userID)
	{
		$record = array(
					   'active' => 1,
					);
		
		$this->db->where('userID', $guest_userID);
		$this->db->update('users', $record); 
	}

	function get_transaxrecords($randnum)
	{
		$this->db->where('randomvalue', $randnum);		

		// grab
		$query = $this->db->get('ha_transax_records', 1);
	...
	function get_transaction_id($transactionCode)
	{
		$this->db->where('transactionCode', $transactionCode);		

		// grab
		$query = $this->db->get('ha_shop_transactions', 1);
	...
	function get_guest()
	{
		if ($guestSession = $this->session->userdata('guest_sess_id'))
		{			
			$query = $this->db->get_where('guests', array('sessionID' => $guestSession), 1);
	...
	function insert_transaction()
	{
		...
		// if($isOversize){
			...
			// $set['postage'] = number_format($cart['postage']*2, 2, '.', '');
			// $set['amount'] = number_format(($cart['subtotal'] + ($cart['postage']*2) + $cart['tax']), 2, '.', '');
		// }
	...
	function insert_guest_transaction()
	{
		$cartSession = $this->session->userdata('cart');
		$cart = $this->load_cart();
		// $isOversize = false;
		...
		// get guest info
		if ($guest = $this->get_guest_info($this->session->userdata('guest_sess_id')))
		{
			// copy guest info to 'user' records..
			// data will be needed: (to complete the order details)
			// this data will also be used to create a user account (for those guests who will register)
			
			$password = '';
			$siteID = '';
			
			// add password (for those guests who will register)
			...
			$guestinfo = array(
			   ...
			   'billingAddress1' => $guest['billingAddress1'] ,
			   'billingAddress2' => $guest['billingAddress2'] ,
			   'billingAddress3' => $guest['billingAddress3'] ,
			   'billingCity' => $guest['billingCity'] ,
			   'billingState' => $guest['billingState'] ,
			   'billingPostcode' => $guest['billingPostcode'] ,
			   'billingCountry' => $guest['billingCountry'] ,
			   'phone' => $guest['phone'] ,
			   'active' => 0 ,
			   'guest' => 1 ,
			   'guest_sessionID' => $this->session->userdata('guest_sess_id') ,
			   'password' => $password ,
			   'siteID' => $siteID ,
			);
			
			// check if there is already an existing copy (if there is.. just update it)
			if ($guestrecord = $this->shop->guestrecord($this->session->userdata('guest_sess_id')))
			{
			    $guest_userID = $guestrecord['userID'];
				$this->db->where('guest_sessionID', $this->session->userdata('guest_sess_id'));
				$this->db->update('users', $guestinfo); 
			}
			
			// if there is no existing copy, create one..
			else {
				$this->db->insert('users', $guestinfo);
				$guest_userID = $this->db->insert_id();
			}
		}
		
		$this->session->set_userdata('guest_userID', $guest_userID);
	...
	function get_guest_info($guest_sessionid)
	{
		// default wheres
		$this->db->where('siteID', $this->siteID);		
		
		$this->db->where('sessionID', $guest_sessionid);
	...
	function guestrecord($guest_sessionid)
	{
		
		$this->db->where('guest_sessionID', $guest_sessionid);
	...
	function add_variation($productID, $type, $variation = '', $varPrice = 0,$name ='', $img='')
	{
	...
	function clear_variations($productID = '')
	{
        ...
        function insertReferenceID($reference_ID,$orderID){
                $this->db->set('reference_id', $reference_ID);
		$this->db->where('transactionCode', $orderID);
                $this->db->where('siteID', $this->siteID);
		$this->db->update('shop_transactions');
        }
	...
	function update_order($transactionID)
	{
		...
		if ($this->session->userdata('transaxTransID')){
		$transaxTransID = $this->session->userdata('transaxTransID');							 
		}
		
		$this->db->set('paid', 1);
		$this->db->set('expiryDate', date("Y-m-d H:i:s", strtotime('+7 days')));
		$this->db->set('transaxTransID', $transaxTransID);
		$this->db->where('transactionID', $transactionID);
		$this->db->update('shop_transactions');
	...
	{	
		...
		$baseStatus = array_shift(split(" ",$output["Status"]));
	...
	function validate_paypalpro()
	{
		...
		// get first line of status
		$baseStatus = array_shift(split(" ",$output["ACK"]));
	...
	{		
		...
		if ($_POST)
		{
			...
			// get first line of status
			$baseStatus = array_shift(split(" ", $this->response_data['Status']));
	...
	{	
    	...
    	function get_images($productID = '',$imgID = ''){
		...
		if($imgID != ''){
			$data = $this->db->get_where('shop_productsImages',array('productID'=>$productID, 'imgID' => $imgID));
			return $data->row_array();
		}else{
			$data = $this->db->get_where('shop_productsImages',array('productID'=>$productID));
