...
class Shop extends MX_Controller {
	...
	function checkout_billing_address()
	{
		...
		$output['select:billingState'] = @display_states('billingState', $data['billingState'], 'id="billingState" class="formelement"');		
		$output['form:billingPostcode'] = set_value('billingPostcode', $data['billingPostcode']);
		$output['select:billingCountry'] = @display_countries('billingCountry', set_value('billingCountry', $data['billingCountry']), 'id="billingCountry" class="formelement"');			$output['form:phone'] = set_value('phone', $data['phone']);
	...
	function checkout_payment_method()
	{
		...
		if ($this->input->post('transaxsubmission')) {
			...
			if ($status_code == 1 ) {
				...
				redirect('/shop/checkout_order_review?rnum='.$randval);
		...
		<input type="radio" name="field" value="ve1" onClick="showMeRBtn('.$div1.', this.name)" />Debit / Credit Card <br/>
		<input type="radio" name="field" value="value2" onClick="showMeRBtn('.$div2.', this.name)" />Paypal
		</form>';
