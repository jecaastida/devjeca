...
class Shop extends MX_Controller {
	...
	function viewproduct($productID)
	{
		...
		$image = $this->uploads->load_image($productID, false, true);
            ...
            if ($image['src']) 
            {
                ...
                $output['product:image-path'] = 'https://goo.gl/Kjcemo';
            }
		...
		if ($similar = $this->shop->get_similar_products($productID, $category['catID'], $this->site->config['headlines']))
		{
			...
			{	
				...
				{
					$similarProductImage['src'] = 'https://goo.gl/Kjcemo';
				}	
				// get images
				if (!$similarProductThumb = $this->uploads->load_image($similarProduct['productID'], true, true))
				{
					$similarProductThumb['src'] = 'https://goo.gl/Kjcemo';
				}
		...
		$output['product:category'] = (isset($category) && $category) ? $category['catName'] : '';
                ...
                if($this->session->userdata('groupID') != "8"){
                    if ($product['sale_price'] != 0) {
                        $tmpStr = "<p class='retail-price crossed-out'>Original Price: <span>" . currency_symbol() . number_format($product['price'], 2) . "</span></p>
	...
	function cart($quantity = '', $id = '')
	{
        ...
        if ($shippingBand== 2){
            ...
            if ($user['state'] == 'LUZ'){
                ...
                $shippingModifier = 1.66666;
            }
            if ($user['state'] == 'VIS'){
                
                $shippingModifier = 2.8;
            }
		...
		$output['upsell:image-path'] = ($image) ? $image['src'] : 'https://goo.gl/Kjcemo';
		$image = ($upsell) ? $this->uploads->load_image($upsell['productID'], true, true) : '';
		$output['upsell:thumb-path'] = ($image) ? $image['src'] : 'https://goo.gl/Kjcemo';
	...
	function checkout()
	{
		...
		if (count($_POST))
		{
            if($this->input->post('payment_method') == 'value1' ){
		...
		if (@$user['country'] == 'PH' && !@$user['postcode'])
		{
		...
		// check a state is set up
		if (@$user['billingCountry'] == 'PH' && !@$user['billingState'])
		{
		...
		// check a zipcode is set up
		if (@$user['billingCountry'] == 'PH' && !@$user['billingPostcode'])
		{
	...
	function checkout_billing_address()
	{
		...
		if ($this->session->userdata('guest_sess_id')) {
			...
			$output['form:email'] .= @form_input('email',set_value('email', $data['email']), 'id="email" class="form-control"');
			...
			$output['form:password'] .= @form_password('password', '', 'id="password" class="form-control"');
			$output['form:password'] .= '<br class="clear" />';
			
			$output['form:confirmPassword'] = @form_label('Confirm Password', 'confirmPassword');
			$output['form:confirmPassword'] .= @form_password('confirmPassword', '', 'id="confirmPassword" class="form-control"');
	...
	function checkout_payment_method()
	{
		...
		<input type="radio" name="payment_method" value="value1" onClick="showMeRBtn('.$div1.', this.name)" />Bank Deposit (BDO) <br/>
	...
	function cart_info_leftbar()
	{
		...
		$output .= "<li class='list-group-item'><span style='width:100px;float:left'>Discount:</span><span style='float:right'>".$discountvalue."</span></li>";
		$output .= "<li class='list-group-item'><span style='width:100px;float:left'>Sub total:</span><span style='float:right'>".$subtotal."</span></li>";
		$output .= "<li class='list-group-item'><span style='width:100px;float:left'>Shipping:</span><span style='float:right'>".$shippingrate."</span></li>";
		
		if ($data['tax'] > 0){
		$output .= "<li class='list-group-item'><span style='width:100px;float:left'>Tax:</span><span style='float:right'>".$statetax."</span></li>";
		}
		$output .= "<li class='list-group-item'><span style='width:100px;float:left'>TOTAL:</span><span style='float:right'>".$carttotal."</span></li>";
		$output .= "</ul>";
		return $output;
	}
	function reset($key = '')
