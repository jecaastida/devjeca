...
<table class="default clear<?php echo ($catID) ? ' order' : ''; ?>">
	<thead>
		<tr>
			...
			<th class="narrow"><?php echo order_link('admin/shop/products'.(($catID) ? '/'.$catID : ''),'price','Retail Price ('.currency_symbol().')', (($catID) ? 5 : 4)); ?></th>
		...
		<tr class="<?php echo (!$product['published']) ? 'draft' : ''; ?>" id="shop_products-<?php echo $product['productID']; ?>">
			...
			<td class="col9 tiny">
				...
				<?php endif; ?>
                        ...
                        <td class="col9 tiny">
				<?php if (in_array('shop_edit', $this->permission->permissions)): ?>	
					<?php echo anchor('/admin/shop/copy_product/'.$product['productID'], 'Copy', 'onclick="return confirm(\'Are you sure you want to copy this product?\')"'); ?>
				<?php endif; ?>
			</td>
		</tr>
	<?php endforeach; ?>
